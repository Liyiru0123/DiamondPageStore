...
-- 以下存储过程用于 Customer 端功能，包含事务处理确保数据一致性
-- =========================================================================
...
-- 输入格式：p_cart_items 为 JSON 格式，例如：[{"sku_id": 1, "quantity": 2}, {"sku_id": 3, "quantity": 1}]
...
BEGIN
    ...
    ELSE
        ...
        read_loop: LOOP
            ...
            batch_loop: WHILE v_remaining > 0 DO
                SELECT batch_id, quantity INTO v_batch_id, v_batch_qty
                FROM inventory_batches
                WHERE sku_id = v_sku_id AND store_id = v_store_id AND quantity > 0
        ...
        -- 增加会员积分（1元 = 1积分）
        UPDATE members
        SET point = point + FLOOR(v_total_amount)
        WHERE member_id = v_member_id;

        -- 记录积分变更
        INSERT INTO point_ledgers (member_id, points_change, change_date, reason)
        VALUES (v_member_id, FLOOR(v_total_amount), NOW(), CONCAT('Order payment: ', p_order_id));
...
-- 3. 输出参数统一使用 p_result_code 和 p_result_message
--    - p_result_code: 1=成功, 0=业务逻辑失败, -1=系统错误
--    - p_result_message: 详细的结果信息
-- 4. 订单支付时自动扣减库存，使用FIFO原则
-- 5. 支付成功后自动增加会员积分并记录到积分账本
-- 6. 支持合并支付多个订单

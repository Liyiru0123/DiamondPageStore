...
*   **Overview & Analytics (业务概览与分析)**:
    经理登录后的首页展示了全连锁店的运营数据分析,方便经理登录上去就能一览各分店的近期销售情况。
    *   **统计卡片**: 实时显示表现最佳的分店、最受欢迎的支付方式以及销售额最高的书籍类别。
    *   **可视化图表**: 通过柱状图直观对比各分店的订单量。
    *   **多维报表**: 展示订单支付方式的使用情况对比，热销书籍各种类贡献率分析，帮助经理进行财务决策和选品优化。

*   **Inventory & Replenishment (库存与补货审批)**:
    经理掌握着库存调拨和补货的最终审批权，这一页允许经理处理员工提交的“补货申请”、查看“库存总览”以及修改书本定价。
    *   **补货审批**: 经理可以处理staff提交的补货申请(初始为pending状态)，对于pending状态的申请，经理可以approve，也可以Reject。approve之后的申请可以标记为completed。每一条记录都可以查看详情。
    对于这些补货申请记录，可以通过分店、申请状态、日期范围以及紧急程度进行筛选，reset按钮可以初始化筛选条件。 
    *   **全盘库存**: 可以按分店或按书籍（SKU）两种维度查看全连锁店的实时库存分布，两个筛选条件下都可以通过SKU，book title和ISBN来查询具体的记录。
    *   **定价管理**: 经理拥有修改书籍零售价格的权限，确保全连锁店价格策略的一致性。此界面可以通过ISBN，book title以及SKU来查询到具体书籍。

*   **Staff Management (员工管理)**:
    经理负责全连锁店的人事行政工作。
    *   **员工列表**: 集中展示所有分店的员工信息，支持按分店和职位（Manager/Staff/Finance）进行筛选，reset按钮可以初始化筛选条件。 
    *   **入职管理**: 点击“Add Staff”可为新员工创建系统账号，分配所属门店和职位。
    *   **信息维护**: 支持修改员工的联系方式、所属门店及职位，并可执行离职注销操作。

*   **User Management (用户管理)**:
    负责维护系统所有账号的安全与状态。
    *   **账号审计**: 查看所有注册用户（包括客户和员工）的最后登录时间和账号状态，可以通过UserID, username, name, 以及email来搜索具体账户。
    *   **安全控制**: 经理可以重置任何用户的密码（重置为系统默认密码），或在必要时禁用/删除违规账号。
    *   **信息维护**：经理可以在edit功能中修改账户名、个人名字。



*   **Supplier Management (供应商管理)**:
    维护书店的供应链体系。
    *   **供应商名录**: 记录所有合作伙伴的联系方式和地址，可以通过name，phone, address以及email来查询具体供应商。
